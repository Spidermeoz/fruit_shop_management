    ...
    return Order.create({
      ...
      items: row.items
        ? row.items.map((item: any) => ({
            ...
            lineDiscountAmount: Number(item.line_discount_amount ?? 0),
            promotionSnapshot: item.promotion_snapshot_json ?? null,
      ...
      const order = await this.models.Order.create(
        {
          ...
          shipping_discount_amount: data.shippingDiscountAmount ?? 0,
          promotion_snapshot_json: data.promotionSnapshot ?? null,
        ...
        await this.models.OrderItem.create(
          {
            ...
            promotion_snapshot_json: item.promotionSnapshot ?? null,
