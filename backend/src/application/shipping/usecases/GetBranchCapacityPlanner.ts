...
export class GetBranchCapacityPlanner {
  constructor(
    private readonly capacityRepo: BranchDeliverySlotCapacityRepository,
    private readonly branchRepo: BranchRepository,
  ) {}

  async execute(input: { deliveryDate: string; branchIds?: number[] }) {
    ...
    const rows = await this.capacityRepo.getPlanner(
      input.deliveryDate,
      branchIds,
    );
    return { deliveryDate: input.deliveryDate, rows };
  }
}
