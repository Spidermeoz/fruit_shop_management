...
export interface OrderRepository {
  ...
  findByUser(id: number, filter: OrderListFilter): Promise<{ rows: Order[]; count: number }>;

  updateStatus(id: number, status: OrderStatus): Promise<void>;

  addDeliveryHistory(orderId: number, status: string, location?: string, note?: string): Promise<void>;
