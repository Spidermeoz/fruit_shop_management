...
const suggestSortOrder = (startTime?: string) => {
  if (!startTime) return "0";
  const hour = parseInt(startTime.split(":")[0], 10);
  if (Number.isNaN(hour)) return "0";
  if (hour < 8) return "10";
  if (hour < 10) return "20";
  if (hour < 12) return "30";
  if (hour < 14) return "40";
  if (hour < 17) return "50";
...
const DeliveryTimeSlotCreatePage: React.FC = () => {
  ...
  const handleAutoFillSortOrder = () => {
    const nextSortOrder = suggestSortOrder(formData.startTime);
    ...
    if (
      ...
      if (Number.isInteger(cutoff) && cutoff > start) {
        nextErrors.cutoffMinutes =
          "Cutoff quá lớn so với giờ bắt đầu của khung giờ.";
      }
    }
        ...
        <div className="xl:col-span-2 space-y-6">
          ...
          <Card>
            ...
            <div className="mt-5 flex flex-wrap gap-2">
              <button
                type="button"
                onClick={handleAutoFillSortOrder}
                className="px-3 py-2 rounded-lg border border-gray-200 text-sm font-medium text-gray-700 hover:bg-gray-50 transition"
              >
                Gợi ý sort order theo giờ bắt đầu
              </button>
            </div>
          </Card>

          {/* Section 4: Cảnh báo vận hành */}
          <Card className="border-amber-200 bg-amber-50/40">
            <div className="flex items-start gap-3">
              <ShieldAlert className="w-5 h-5 text-amber-600 mt-0.5" />
              <div>
                <h3 className="text-sm font-bold text-amber-800">
                  Lưu ý khi tạo template slot
                </h3>
                <ul className="mt-2 space-y-1 text-sm text-amber-700 list-disc list-inside">
                  <li>
                    Template này sẽ được dùng để bật slot cho nhiều chi nhánh.
                  </li>
                  <li>
                    Nếu đổi cutoff hoặc giới hạn gốc sau này, planner branch có
                    thể bị ảnh hưởng.
                  </li>
                  <li>
                    Nên tạo mã dễ đọc và ổn định để thuận tiện copy / rollout.
                  </li>
                </ul>
              </div>
            </div>
          ...
          <div className="sticky top-24 space-y-6">
            ...
            <Card className="bg-gray-50 dark:bg-gray-800/50 border border-gray-200 dark:border-gray-700">
              ...
              <p className="text-xs text-gray-600 dark:text-gray-400 leading-relaxed">
                Sau khi tạo template thành công, bạn có thể chuyển sang mục{" "}
                <strong>Khung giờ chi nhánh</strong> để bật slot này cho từng
                branch hoặc rollout hàng loạt.
              </p>
            </Card>

            <Card className="bg-white border border-gray-200 dark:border-gray-700">
              <h3 className="text-sm font-bold text-gray-800 dark:text-gray-200 mb-3 flex items-center gap-2">
                <ListOrdered className="w-4 h-4" /> Gợi ý thao tác nhanh
              </h3>
              <div className="space-y-2 text-xs text-gray-600 dark:text-gray-400">
                <div className="rounded-lg bg-gray-50 dark:bg-gray-800 p-3">
                  1. Chọn giờ bắt đầu / kết thúc
                </div>
                <div className="rounded-lg bg-gray-50 dark:bg-gray-800 p-3">
                  2. Tự điền tên & mã
                </div>
        ...
        <div className="max-w-7xl mx-auto flex flex-col sm:flex-row items-center justify-between gap-4">
          ...
          <div className="flex gap-3 w-full sm:w-auto">
            ...
            >
              {loading ? (
                <>
                  <Loader2 className="w-5 h-5 animate-spin" /> Đang tạo slot...
                </>
              ) : (
                <>
                  <Save className="w-5 h-5" /> Lưu template slot
                </>
              )}
            </button>
          </div>
